...
def alignment_dataset(batch_size, sample_size=1000, subset_size=500):
    ...
    # Randomly sample 'sample_size' train images
    indices = torch.randperm(len(trainset))[:sample_size]
    ...
    # Randomly sample 'subset_size' train images
    indices = torch.randperm(len(trainset))[:subset_size]
    subset_trainset = Subset(trainset, indices)
    sub_trainloader = DataLoader(subset_trainset, batch_size=batch_size, shuffle=True, num_workers=2)

    return trainloader, sub_trainloader 
